...
Разработанные сайты на каждом фреймворке выглядят одинаково, за исключением портов на которых они работают. Далее будут представлены изображения, относящиеся к реализации сайта на `Flask`:
...
| Framework | Движок БД |     ORM    | Миграция БД | Рендер страниц | Админ. панель |
|-----------|-----------|------------|-------------|----------------|---------------|
|   Flask   |  sqlite   |    нет     |     нет     |     Jinja2     |      нет      |
|  FastAPI  |  sqlite   | SQLAlchemy |   alembic   |     Jinja2     |      нет      |
|   Django  |  sqlite   | Django ORM |   Django    |       DTL      |      да       |
